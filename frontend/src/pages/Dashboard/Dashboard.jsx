...
export default function Dashboard() {
  const { user, recordLearningSession } = useGame()
  ...
  return (
    <div className={styles.dashboardPage}>
      <section className={styles.topStatus}>
        <div>
          <h2>Happy Learning, {user.name}</h2>
          <p>Role(s): {user.interestedRoles?.join(', ') || 'Not set yet'}</p>
        </div>

        <div className={styles.streakSummary}>
          <Badge variant="streak">🔥 Streak: {user.streakState?.current ?? user.streakDays}</Badge>
          <Badge variant="primary">🎯 Daily Goal: {user.dailyTargetMinutes} min</Badge>
          <Badge variant="leaderboard">⏱ Today: {user.dailyLearning?.minutes ?? 0} min</Badge>
